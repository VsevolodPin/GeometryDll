	...
	for (int i = 0; i < accuracy1 - 1; i++)
	{
		...
		for (int j = 0; j < accuracy2 - 1; j++)
		{
					...
					(e1 < p2[j].e1 && e1 > p2[j + 1].e1)))
			{
				...
				std::cout << "Точка пересечения, точно соответствующая уравнению кривой 1: " << realPoint1.e1 << " " << realPoint1.e2 << "\n";
				std::cout << "Точка пересечения, точно соответствующая уравнению кривой 2: " << realPoint2.e1 << " " << realPoint2.e2 << "\n";
			}
		}
	}
