...
public:
	...
	// 
	virtual const int basePointsSize() = 0;
...
public:
	...
	// 
	const int basePointsSize() override
	{
...
DllExport vector<Point2D> FindCrossPoints(Curve* curve1, Curve* curve2, double eps = 1e-9)
{
	...
	// Функция, которая будет минимизароваться - расстояние между точками кривых
	auto metric = [](Point2D p1, Point2D p2)->double {return Vector2D(p1.e1, p1.e2, p2.e1, p2.e2).GetLength(); };
	int hypothesisCountOfPoints = curve1->basePointsSize() > curve2->basePointsSize() ? curve1->basePointsSize() - 1 : curve2->basePointsSize() - 1;
	// Что-то вроде локализации корней - по-прежнему не совсем понятно, как это сделать
